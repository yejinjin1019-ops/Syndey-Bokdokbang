...
export function GuidesPreview() {
  ...
  return (
    <section style={{ backgroundColor: COLORS.tangerine, borderTop: `1px solid rgba(25,26,23,0.14)` }}>
      <div className="max-w-[1400px] mx-auto px-5 md:px-10 py-20 md:py-28">
        <Reveal>
          <div className="mb-5">
            <SectionLabel font={body} dark>{t("가이드 & 리소스", "Guides & Resources")}</SectionLabel>
          </div>
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-4 border-l border-t" style={{ borderColor: "rgba(25,26,23,0.2)" }}>
          {GUIDE_CATEGORIES.map((cat, i) => (
            <Reveal
              key={cat.id}
              delay={i * 80}
              className="border-r border-b p-8 lg:p-9"
              style={{ borderColor: "rgba(25,26,23,0.2)" }}
            >
              <Link to="/guides" className="group block h-full" style={{ textDecoration: "none" }}>
                <div className="flex items-center justify-between mb-6">
                  <span
                    className="font-semibold"
                    style={{ fontFamily: display, color: COLORS.ink, fontSize: "13px", letterSpacing: "0.02em" }}
                  >
                    {String(i + 1).padStart(2, "0")}
                  </span>
                  <div className="w-5 h-px" style={{ backgroundColor: COLORS.green }} />
                </div>
                ...
                <p className="text-[13px] leading-relaxed mb-5" style={{ color: "rgba(25,26,23,0.72)", fontFamily: body }}>
